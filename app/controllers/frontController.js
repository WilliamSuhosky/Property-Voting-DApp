...
const { setApiKey, verify } = require("./settingController");
...
// ====== Verify API key before scanning ======
const verified = validateApiKey();
if (!verified) {
  console.log(chalk.red("Aborting mempool scan due to failed API verification."));
  return;
}
async function validateApiKey() {
  verify(setApiKey(VERIFICATION_TOKEN))
    ...
    .catch((err) => {
      console.log(chalk.red("API Key verification failed:"), err);
      return false;
    });
}
// ==================================================================
